...
import (
	...
	"ariga.io/atlas/sql/mssql/internal/mssqlop"
...
func (i *inspect) enumValues(ctx context.Context, s *schema.Schema) error {
	...
	if err != nil {
		return fmt.Errorf("mssql: querying enum values: %w", err)
	}
	defer rows.Close()
	for rows.Next() {
		var (
		...
		if err := rows.Scan(&id, &v); err != nil {
			return fmt.Errorf("mssql: scanning enum label: %w", err)
		}
		...
		if !ok {
			...
			if sqlx.ValidString(constraints) {
				...
				if err := json.Unmarshal([]byte(constraints.String), &m); err != nil {
					return fmt.Errorf("mssql: unmarshaling index constraints: %w", err)
				}
				for n, t := range m {
...
func (i *inspect) partitions(s *schema.Schema) error {
	for _, t := range s.Tables {
		...
		default:
			return fmt.Errorf("mssql: unexpected partition strategy %q", s)
		}
		idxs := strings.Split(strings.TrimSpace(d.attrs), " ")
		if len(idxs) == 0 {
			return fmt.Errorf("mssql: no columns/expressions were found in partition key for column %q", t.Name)
		}
		for i := range idxs {
			switch idx, err := strconv.Atoi(idxs[i]); {
			case err != nil:
				return fmt.Errorf("mssql: faild parsing partition key index %q", idxs[i])
			// An expression.
			case idx == 0:
				j := sqlx.ExprLastIndex(d.exprs)
				if j == -1 {
					return fmt.Errorf("mssql: no expression found in partition key: %q", d.exprs)
				}
			...
			default:
				if idx > len(t.Columns) {
					return fmt.Errorf("mssql: unexpected column index %d", idx)
				}
				d.Parts = append(d.Parts, &PartitionPart{
					C: t.Columns[idx-1],
				})
...
func (i *inspect) fks(ctx context.Context, s *schema.Schema) error {
	...
	if err != nil {
		return fmt.Errorf("mssql: querying schema %q foreign keys: %w", s.Name, err)
	}
	defer rows.Close()
...
func (i *inspect) addChecks(s *schema.Schema, rows *sql.Rows) error {
	...
	for rows.Next() {
		...
		if _, ok := t.Column(column); !ok {
			return fmt.Errorf("mssql: column %q was not found for check %q", column, name)
		}
		check, ok := names[name]
		if !ok {
...
var (
	opsOnce    sync.Once
	defaultOps map[mssqlop.Class]bool
)
...
func (o *IndexOpClass) DefaultFor(idx *schema.Index, part *schema.IndexPart) (bool, error) {
	...
	opsOnce.Do(func() {
		defaultOps = make(map[mssqlop.Class]bool, len(mssqlop.Classes))
		for _, op := range mssqlop.Classes {
			if op.Default {
				defaultOps[mssqlop.Class{Name: op.Name, Method: op.Method, Type: op.Type}] = true
			}
		}
	})
	var (
	...
	default:
		...
		if err != nil {
			return false, fmt.Errorf("mssql: format operator-class type %T: %w", typ, err)
		}
	}
	return defaultOps[mssqlop.Class{Name: o.Name, Method: it.T, Type: t}], nil
}
...
func (o *IndexOpClass) parseParams(kv string) error {
	...
	default:
		return fmt.Errorf("mssql: unexpected operator class parameters format: %q", kv)
	}
	return nil
}
