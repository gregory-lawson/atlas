...
func FormatType(t schema.Type) (string, error) {
	...
	case *schema.EnumType:
		if t.T == "" {
			return "", errors.New("mssql: missing enum type name")
		}
		f = t.T
	...
	case *schema.DecimalType:
		...
		case p == 0 && s > 0:
			return "", fmt.Errorf("mssql: decimal type must have precision between 1 and 1000: %d", p)
		case s == 0:
	...
	case *SerialType:
		...
		case TypeSerial8:
			f = TypeBigSerial
		default:
			return "", fmt.Errorf("mssql: unexpected serial type: %q", t.T)
		}
	...
	case *RangeType:
		...
		default:
			return "", fmt.Errorf("mssql: unsupported range type: %q", t.T)
		}
	case *OIDType:
		switch f = strings.ToLower(t.T); f {
		...
		default:
			return "", fmt.Errorf("mssql: unsupported object identfier type: %q", t.T)
		}
	case *TextSearchType:
		if f = strings.ToLower(t.T); f != TypeTSVector && f != TypeTSQuery {
			return "", fmt.Errorf("mssql: unsupported text search type: %q", t.T)
		}
...
func parseColumn(s string) (*columnDesc, error) {
	...
	case TypeTime, TypeTimeTZ, TypeTimestamp, TypeTimestampTZ:
		...
		if len(parts) > 1 && reDigits.MatchString(parts[1]) {
			...
			if err != nil {
				return nil, fmt.Errorf("mssql: parse time precision %q: %w", parts[1], err)
			}
			p = i
	...
	case TypeInterval:
		...
		if matches[2] != "" {
			...
			if err != nil {
				return nil, fmt.Errorf("mssql: parse interval precision %q: %w", parts[1], err)
			}
			c.timePrecision = &i
		}
	default:
...
func parseCharParts(parts []string, c *columnDesc) error {
	...
	if err != nil {
		return fmt.Errorf("mssql: parse size %q: %w", parts[0], err)
	}
	c.size = size
	return nil
}
...
func parseBitParts(parts []string, c *columnDesc) error {
	...
	size, err := strconv.ParseInt(parts[0], 10, 64)
	if err != nil {
		return fmt.Errorf("mssql: parse size %q: %w", parts[1], err)
	}
	c.size = size
	return nil
}
