...
import (
	...
	"ariga.io/atlas/sql/mssql/mssqlcheck"
	_ "ariga.io/atlas/sql/mssql/mssqlcheck"
...
`), &cfg, nil)
	...
	require.NoError(t, err)
	az, err := mssqlcheck.NewConcurrentIndex(cfg.Remain())
	require.NoError(t, err)
...
func TestDataDepend_MightFail(t *testing.T) {
						...
						Changes: schema.Changes{
							&schema.ModifyTable{
								T: schema.NewTable("users").
									...
									AddColumns(
										schema.NewIntColumn("a", mssql.TypeInt),
										schema.NewIntColumn("b", mssql.TypeInt),
									),
								Changes: []schema.Change{
									&schema.AddColumn{C: schema.NewTimeColumn("b", mssql.TypeInt)},
								},
							},
						},
					},
	...
	azs, err := sqlcheck.AnalyzerFor(mssql.DriverName, nil)
	require.NoError(t, err)
