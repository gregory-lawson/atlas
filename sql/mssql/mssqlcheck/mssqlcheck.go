...
func addNotNull(p *datadepend.ColumnPass) (diags []sqlcheck.Diagnostic, err error) {
	tt, err := mssql.FormatType(p.Column.Type.Type)
	if err != nil {
		return nil, err
	}
...
func init() {
	sqlcheck.Register(mssql.DriverName, func(r *schemahcl.Resource) ([]sqlcheck.Analyzer, error) {
		...
		for _, c := range sc.Changes {
			...
			for i := range m.Changes {
				...
				case *schema.AddIndex:
					switch hasC := sqlx.Has(mc.Extra, &mssql.Concurrently{}); {
